...
def load_all_trackon_east_rates():
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute("""
        SELECT zone, slab_weight, rate, is_per_kg
        FROM trackon_east_rates
    """)
...
def calculate_trackon_east_rate(state, weight, rate_dict):
    ...
    if state_clean == "bihar":
        zone = "Bihar"

    elif state_clean == "jharkhand":
        zone = "Jharkhand"
